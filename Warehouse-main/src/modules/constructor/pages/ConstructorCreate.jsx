...
import { createWarehouse } from '../store/actions';
...
function createStock() {
    ...
    const addRack = () => {
      if (countChange === '2') {
        if (directionVal === '1') {
          const rackCopies = [];
            ...
            for (let i = 0; i < rectCount.current.value; i++) {
              const copy = { ...Rack,width: gridSizeValue, height: gridSizeValue, rack_id: Rack.shelf_level > 1 ? `${Rack.rack_id}.${(i+1)}.${Rack.shelf_level}` : `${Rack.rack_id}.${(i+1)}`, x: posX + i*gridSizeValue, y: posY};
          ...
          for (let i = 0; i < rectCount.current.value; i++) {
            const copy = { ...Rack, width: gridSizeValue, height: gridSizeValue, rack_id:  Rack.shelf_level > 1 ? `${Rack.rack_id}.${(i+1)}.${Rack.shelf_level}` : `${Rack.rack_id}.${(i+1)}`, y: posY + i*gridSizeValue, x: posX};
            rackCopies.push(copy);
          }
        ...
        setRectangle(rect => [...rect, {...Rack, x: posX, y: posY, width: gridSizeValue, height: gridSizeValue}])
      }
    }
    ...
    const createSklad = () => {
      ...
      console.log(constructorJSON);
      // rectangle.forEach(obj => {
      //   constructorJSON.zones.forEach(zone => {
      //     if (zone.title === obj.zone) {
      //       zone.shelves.push(obj);
      //     }
      //   });
      // });
      dispatch(createWarehouse(constructorJSON))
    }
    ...
    useEffect( () => {
      const handleKeyDown = (e) => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === "v") {
          const newRectangles = [...rectangle];
          const obj = {
            ...rectangle[figureIndex],
            x: posX,
            y: posY,
          }
          newRectangles.push(obj)
          setRectangle(newRectangles)
    ...
    }, [figureIndex, rectangle, posX, posY])
        ...
        <div className="app">
            ...
            <div className="plane-right">
                <div className="plane-functions">
                    ...
                    <button className="add-rack" onClick={showRack}>Добавить место хранения</button>
                ...
                <div className={gridState ? 'input-area grid-input active' : 'input-area grid-input'}>
                    ...
                    <div>
                        <input type="number" ref={gridSizeInput} placeholder='Укажите число. Значение равно ДМ,изменяется размер плитки.' min={20} max={60}/>
                ...
                <div className={rackState ? 'input-area rack-input active' : 'input-area rack-input'}>
                    <div>
                      <p>Одноуровневое палетоместо или Стеллажная система</p>
                      <select onChange={handleCountChange}>
                        <option value="1">Одноуровневое палетоместо</option>
                        <option value="2">Стеллажная система</option>
                      </select>
                    </div>
                    { countChange === '2' ? <>
                      <div>
